...
const KNOWN_PROVIDERS: &[(&str, ProviderKind)] = &[
    ("anthropic", ProviderKind::AineerApi),
    ("claude", ProviderKind::AineerApi),
...
fn provider_display_name(kind: ProviderKind) -> &'static str {
    match kind {
        ProviderKind::AineerApi => "Anthropic",
...
pub fn build_credential_chain(kind: ProviderKind, config: &RuntimeConfig) -> CredentialChain {
    match kind {
        ProviderKind::AineerApi => {
            let oauth_config = config.oauth().cloned().unwrap_or_default();
...
fn resolve_provider_and_chain(
    provider: Option<&str>,
) -> CliResult<(ProviderKind, RuntimeConfig, CredentialChain)> {
    let kind = match provider {
        Some(name) => resolve_provider_name(name)?,
        None => ProviderKind::AineerApi,
    };
...
pub fn run_login(provider: Option<&str>, source: Option<&str>) -> CliResult<()> {
    ...
    if let Some(source_id) = source {
        ...
        return Ok(resolver.login()?);
    }

    if kind == ProviderKind::AineerApi {
        return run_aineer_oauth_login(&config);
    }
    ...
    Ok(login_sources[0].login()?)
}

pub fn run_logout(provider: Option<&str>, source: Option<&str>) -> CliResult<()> {
...
pub fn run_status(provider: Option<&str>) -> CliResult<()> {
    let cwd = env::current_dir()?;
    let config = ConfigLoader::default_for(&cwd).load()?;
    ...
    match resolver.resolve() {
        ...
        _ => {
            let hint = if resolver.id() == "claude-code" {
                "Run `claude login` in Claude Code first, then Aineer will auto-discover the credentials."
            } else {
                "This credential source is auto-discovered and cannot be set up interactively through Aineer."
            };
...
// ---------------------------------------------------------------------------
// Aineer OAuth browser login flow
// ---------------------------------------------------------------------------

fn run_aineer_oauth_login(config: &RuntimeConfig) -> CliResult<()> {
    ...
    println!("Starting Aineer OAuth login...");
    ...
    println!("Aineer OAuth login complete.");
    Ok(())
}
...
fn wait_for_oauth_callback(port: u16) -> CliResult<aineer_engine::OAuthCallbackParams> {
    ...
    let callback = OAuthCallbackParams::from_request_target(target)
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
    let body = if callback.error.is_some() {
        "Aineer OAuth login failed. You can close this window."
    } else {
        "Aineer OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
Local models (no API key needed):\n\
 Ollama               ollama serve + aineer --model ollama/qwen3-coder\n\
 LM Studio            aineer --model lmstudio/model-name\n\n\
