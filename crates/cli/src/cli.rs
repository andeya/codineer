...
const SUBCOMMAND_HELP: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "aineer system-prompt [--cwd PATH] [--date YYYY-MM-DD]",
    ),
    (
        "login",
        "Start the login flow for a provider.",
        "aineer login [<provider>] [--source <id>]",
    ),
    (
        "logout",
        "Clear saved credentials for a provider.",
        "aineer logout [<provider>] [--source <id>]",
    ),
    (
        "status",
        "Show authentication status.",
        "aineer status [<provider>]",
    ),
    (
        "models",
        "List available models (fetched dynamically via v1/models).",
        "aineer models [<provider>]",
    ),
    (
        "providers",
    ...
    (
        ...
        "aineer config <set|get|list> [<key>] [<value>]",
    ),
    (
        "init",
        "Scaffold .aineer/ directory with settings, gitignore, and AINEER.md.",
        "aineer init",
    ),
    (
        "update",
        "Check for Aineer updates from GitHub releases.",
        "aineer update",
    ),
];
...
pub(crate) fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push(
            "  Try              Start `aineer` to use interactive slash commands".to_string(),
        );
        lines.push(
            "  Tip              Resume-safe commands also work with `aineer --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
pub(crate) fn default_permission_mode() -> PermissionMode {
    env::var("AINEER_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn parse_config_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match subcmd {
        "set" => {
            let key = args
                .get(1)
                .ok_or("usage: aineer config set <key> <value>")?;
            let value = args
                .get(2)
                .ok_or("usage: aineer config set <key> <value>")?;
        ...
        other => Err(format!(
            "unknown config subcommand: {other}\nusage: aineer config <set|get|list>"
        )),
    }
}
