...
fn atomic_write(path: &Path, content: &[u8]) -> io::Result<()> {
    ...
    let tmp_path = parent.join(format!(".aineer-tmp-{}-{}.tmp", std::process::id(), seq));
...
fn workspace_root() -> io::Result<PathBuf> {
    if let Ok(override_root) = std::env::var("AINEER_WORKSPACE_ROOT") {
...
mod tests {
    ...
    fn workspace_dir() -> std::path::PathBuf {
        std::env::temp_dir().join("aineer-test-workspace")
    }
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        workspace_dir().join(format!("aineer-native-{name}-{unique}"))
    }
    ...
    fn allow_temp_workspace() {
        ...
        std::env::set_var("AINEER_WORKSPACE_ROOT", ws);
    }
    ...
    fn rejects_write_outside_workspace() {
        allow_temp_workspace();
        let sentinel = std::env::temp_dir().join("aineer-test-outside-sentinel");
