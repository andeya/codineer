...
fn prepare_command(
    ...
    #[cfg(windows)]
    {
        ...
        if sandbox_status.filesystem_active {
            let aineer_dir = crate::aineer_runtime_dir(cwd);
            prepared.env("USERPROFILE", aineer_dir.join("sandbox-home"));
            prepared.env("TEMP", aineer_dir.join("sandbox-tmp"));
            prepared.env("TMP", aineer_dir.join("sandbox-tmp"));
        }
        apply_proxy_env(&mut prepared);
        prepared
    }
    #[cfg(not(windows))]
    {
        ...
        if sandbox_status.filesystem_active {
            let aineer_dir = crate::aineer_runtime_dir(cwd);
            prepared.env("HOME", aineer_dir.join("sandbox-home"));
            prepared.env("TMPDIR", aineer_dir.join("sandbox-tmp"));
        }
        apply_proxy_env(&mut prepared);
        prepared
    }
...
fn prepare_sandbox_dirs(cwd: &std::path::Path) {
    let aineer_dir = crate::aineer_runtime_dir(cwd);
    let _ = std::fs::create_dir_all(aineer_dir.join("sandbox-home"));
    let _ = std::fs::create_dir_all(aineer_dir.join("sandbox-tmp"));
}
