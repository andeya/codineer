...
fn rejects_non_object_settings_files() {
    ...
    let home = root.join("home").join(".aineer");
...
fn loads_and_merges_config_files_by_precedence() {
    ...
    let cwd = root.join("project");
    let home = root.join("home").join(".aineer");
    fs::create_dir_all(cwd.join(".aineer")).expect("project config dir");
    fs::create_dir_all(&home).expect("home config dir");
    ...
    assert_eq!(AINEER_SETTINGS_SCHEMA_NAME, "SettingsSchema");
...
fn parses_credentials_config() {
    ...
    fs::write(
        home.join("settings.json"),
        r#"{"credentials":{"defaultSource":"aineer-oauth","autoDiscover":true,"claudeCode":{"enabled":false}}}"#,
    )
    .expect("write settings");

    let loaded = ConfigLoader::new(&cwd, &home).load().expect("load");
    let cred = loaded.credentials();
    assert_eq!(cred.default_source.as_deref(), Some("aineer-oauth"));
