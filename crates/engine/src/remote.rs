...
pub const DEFAULT_REMOTE_BASE_URL: &str = "https://api.aineer.dev";
#[cfg(target_os = "linux")]
pub const DEFAULT_SESSION_TOKEN_PATH: &str = "/run/aineer/session_token";
#[cfg(target_os = "macos")]
pub const DEFAULT_SESSION_TOKEN_PATH: &str = "/tmp/aineer/session_token";
#[cfg(windows)]
...
impl RemoteSessionContext {
    ...
    pub fn from_env_map(env_map: &BTreeMap<String, String>) -> Self {
        Self {
            enabled: env_truthy(env_map.get("AINEER_REMOTE")),
            session_id: env_map
                .get("AINEER_REMOTE_SESSION_ID")
                .filter(|value| !value.is_empty())
                .cloned(),
            base_url: env_map
                .get("AINEER_REMOTE_BASE_URL")
                .filter(|value| !value.is_empty())
                .cloned()
...
impl UpstreamProxyBootstrap {
    ...
    pub fn from_env_map(env_map: &BTreeMap<String, String>) -> Self {
        ...
        Self {
            remote,
            upstream_proxy_enabled: env_truthy(env_map.get("AINEER_UPSTREAM_PROXY_ENABLED")),
            token_path,
            ca_bundle_path,
            system_ca_path,
            token,
...
mod tests {
    ...
    fn remote_context_reads_env_state() {
        let env = BTreeMap::from([
            ("AINEER_REMOTE".to_string(), "true".to_string()),
            (
                "AINEER_REMOTE_SESSION_ID".to_string(),
                "session-123".to_string(),
            ),
            (
                "AINEER_REMOTE_BASE_URL".to_string(),
    ...
    fn bootstrap_fails_open_when_token_or_session_is_missing() {
        let env = BTreeMap::from([
            ("AINEER_REMOTE".to_string(), "1".to_string()),
            (
                "AINEER_UPSTREAM_PROXY_ENABLED".to_string(),
                "true".to_string(),
            ),
        ]);
    ...
    fn bootstrap_derives_proxy_state_and_env() {
        ...
        let env = BTreeMap::from([
            ...
            (
                "AINEER_SESSION_TOKEN_PATH".to_string(),
                token_path.to_string_lossy().into_owned(),
            ),
            (
                "AINEER_CA_BUNDLE_PATH".to_string(),
    ...
    fn helper_outputs_match_expected_shapes() {
        ...
        assert!(no_proxy_list().contains("aineer.dev"));
