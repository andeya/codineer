...
pub(crate) fn download_policy(
  ...
  unsafe {
    ...
    if let Some(started_fn) = started_fn {
      ...
      match started_fn(url_string, &mut download_destination) {
        true => {
...
pub(crate) fn download_did_finish(this: &WryDownloadDelegate, download: &WKDownload) {
  unsafe {
    ...
    let url_string = original_request
      .URL()
      .and_then(|u| u.absoluteString())
      .map(|s| s.to_string())
      .unwrap_or_default();
    if let Some(completed_fn) = this.ivars().completed.clone() {
      completed_fn(url_string, None, true);
    }
  }
}
...
pub(crate) fn download_did_fail(
  ...
  unsafe {
    ...
    let url_string = original_request
      ...
      .unwrap_or_default();
    if let Some(completed_fn) = this.ivars().completed.clone() {
      completed_fn(url_string, None, false);
    }
  }
}
